...
export const  testAPI={
  ...
  getAphorism(id:number|null=null,rand:boolean|null=null,col:number|null=null,type:string|null=null,nottype:string|null=null,obscene:boolean|null=null){
   // console.log(`aphorism?`+(id?`&id=`+encodeURI(`'${id}'`):``)+(rand?`&rand`:``)+(col?`&col=`+encodeURI(`${col}`):``)+(type?`&type=`+encodeURI(`'${type}'`):``) )
    return instanceTest.get<IAphorism[]>(`aphoris1m?`+(id?`&id=`+encodeURI(`'${id}'`):``)+(rand?`&rand`:``)+(obscene?`&obscene`:``)+(col?`&col=`+encodeURI(`${col}`):``)+(type?`&type=`+encodeURI(`${type}`):``)+(nottype?`&nottype=`+encodeURI(`${nottype}`):``) ) },

  getAphorismTypes(){
       return instanceTest.get<IAphorismType[]>(`aphorismtypes` ) },
}
